...
bool save_cont(double, long, int, int, int, const accumulator&, const accumulator&,
	       const long[2]);
bool chrg_cont(double&, long&, int&, int&, int&, accumulator&, accumulator&, long[2]);
...
bool pos_vel(const Dynvec<Dynvec<Spherocyl> >&, const Dynvec<int>&, double, int, int);
...
bool clust_conf(Dynvec<Dynvec<Spherocyl> >&, const part_num&, const double[3], double, 
		double, const gsl_rng*, long[2], bool);
