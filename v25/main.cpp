...
// // *******
  ...
  if(signal == 0){                                     // cout<<"sig 0"<<endl;
    ...
    //charge counters
    if(! chrg_cont(Time, cont, Ntcf, Ntcv, mcont, AcT, AcTsq)){
      Err.open("Error.dat", ios::app);
...
//       if(step >= pre_steps) p_check = true;
      clust_conf(part, NP, L, min_s, dmax, ran, p_check);
    }
...
//       }
	...
	return 1;
      ...
      //print counters//
      if(! save_cont(Time, cont, Ntcf, Ntcv, mcont, AcT, AcTsq)){
	Err.open("Error.dat", ios::app);
	Err << "problem opening counters.dat\n";
	Err.close();
      }
